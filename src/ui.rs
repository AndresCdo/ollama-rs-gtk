...
pub fn build_ui(application: &gtk::Application) {
    ...
    let open_button: gtk::ToolButton = builder.object("open_button").unwrap();
    let save_button: gtk::ToolButton = builder.object("save_button").unwrap();
    ...
    let markdown_view: gtk::ScrolledWindow = builder.object("scrolled_window_right").unwrap();
    ...
    let file_save: gtk::FileChooserDialog = builder.object("file_save").unwrap();
    ...
    open_button.connect_clicked(
        clone!(@strong file_open, header_bar, text_buffer => move |_| {
            ...
            if file_open.run() == gtk::ResponseType::Ok {
                if let Some(file) = file_open.file() {
                    if let Some(path) = file.path() {
                        set_title(&header_bar, &path);
    ...
    save_button.connect_clicked(clone!(@strong file_save, text_buffer => move |_| {
        ...
        if file_save.run() == gtk::ResponseType::Ok {
            if let Some(file) = file_save.file() {
                if let Some(path) = file.path() {
